...
回顾布朗运动的平稳性(stationary): 对所有 $0 \le t_i < t_{i+1}$ , $B (t_{i+1}) - B (t_i)$ 的概率分布是均值为0方差为 $t_{i+1}-t_i$ 的正态分布, 也就是 $N(0, (t_{i+1}-t_i)^2)$.

并且正态分布 $X \sim N(0, \sigma^2)$的期望是 $E(X^2) = \sigma^2$, 那么:

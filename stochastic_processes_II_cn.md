...
了解了连续可微函数的二次变分, 回到布朗运动, 它虽然是连续的, 但是不可微, 也就不是光滑的(smooth). 
...
回顾布朗运动的平稳性(stationary), 对所有 $0 \le t_i < t_{i+1}$ , $B (t_{i+1}) - B (t_i)$ 的概率分布是均值为0方差为 $t_{i+1}-t_i$ 的正态分布, 也就是 $N(0, t_{i+1}-t_i)$. 并且正态分布 $X \sim N(\mu, \sigma^2)$ 平方的期望是 $E(X^2) = \mu^2 + \sigma^2$, 那么:

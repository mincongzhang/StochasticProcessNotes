...
t从整数(integer)换成实数(real number), 就可以让随机过程从离散变成了连续, 连续随机过程(continuous-time stochastic process)可表示为: 
...
那么对于处于某个概率空间(probability space)上的随机变量, 我们用概率密度函数(probability density function, PDF)来描述其概率分布:
